...
resource "helm_release" "cilium" {
  ...
  values = [file("${path.root}/../../helm-values/cilium.yaml")]

  # k8sServiceHost/Port are set in helm-values/cilium.yaml (localhost:7445 KubePrism)
...
resource "helm_release" "longhorn" {
  ...
  values = [file("${path.root}/../../helm-values/longhorn.yaml")]
