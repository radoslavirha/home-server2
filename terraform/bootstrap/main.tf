locals {
  ...
  kubeconfig_path  = "${local.credentials_dir}/kubeconfig"
  talosconfig_path = "${local.credentials_dir}/talosconfig"

  # Derived API endpoint — prefer VIP if set, otherwise fall back to first controlplane IP
...
data "talos_machine_configuration" "controlplane" {
  ...
  config_patches = compact(concat(
    [
      file("${path.root}/../../talos/patches/cilium.yaml"),
      yamlencode({
        machine = {
          install = {
      ...
      lookup(var.longhorn_disks, var.controlplane_ips[count.index], "") != "" ? yamlencode({
        machine = {
          disks = [{ device = lookup(var.longhorn_disks, var.controlplane_ips[count.index], ""), partitions = [{ mountpoint = "/var/lib/longhorn" }] }]
          kubelet = {
            extraMounts = [{ destination = "/var/lib/longhorn", type = "bind", source = "/var/lib/longhorn", options = ["bind", "rshared", "rw"] }]
          }
        }
      }) : "",
    ],
    length(var.worker_ips) == 0 ? [file("${path.root}/../../talos/patches/scheduling.yaml")] : []
  ))
}
...
data "talos_machine_configuration" "worker" {
  ...
  config_patches = compact([
    ...
    lookup(var.longhorn_disks, var.worker_ips[count.index], "") != "" ? yamlencode({
      machine = {
        disks = [{ device = lookup(var.longhorn_disks, var.worker_ips[count.index], ""), partitions = [{ mountpoint = "/var/lib/longhorn" }] }]
        kubelet = {
          extraMounts = [{ destination = "/var/lib/longhorn", type = "bind", source = "/var/lib/longhorn", options = ["bind", "rshared", "rw"] }]
        }
      }
...
# ── 7. Write credentials to disk (gitignored) ───────────────────────────────
